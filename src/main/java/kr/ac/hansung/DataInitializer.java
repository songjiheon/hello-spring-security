...
public class DataInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        if (productRepository.count() == 0) {
                ...
                productRepository.save(new Product("MySQL 성능 최적화",            33000, "인덱스·쿼리 튜닝 실전 가이드",          20));                    productRepository.save(new Product("Redis 캐싱 전략",             29000, "캐시 설계와 세션 관리 실습",             10));
                ...
                productRepository.save(new Product("애플 맥북 프로 M4",          2990000, "M4 칩 탑재 고성능 노트북",               0));                    productRepository.save(new Product("소니 WH-1000XM6",             420000, "업계 최고 노이즈 캔슬링 헤드폰",         22));
